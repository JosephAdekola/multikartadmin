...
import { postNewProduct } from '../utils/apiCalls/products'
...
export default function AddProducts() {
    ...
    const addProductHandler = async(e) => {
        e.preventDefault()
        const payload = {
            images: [image1.length > 5 && (image1), 
                    image2.length > 5 && (image2), 
                    image3.length > 5 && (image3), 
                    image4.length > 5 && (image4), 
                    image5.length > 5 && (image5)].filter(Boolean),
            title: title,
            price: price,
            sizes: size,
            colour: colors,
        ...
        try {
            const res = await postNewProduct(payload)
            if (res) {
                console.log(res);
        ...
        <div>
            ...
            <div className=' p-5 bg-white '>
                ...
                <div className=' my-5 '>
                    ...
                    {<FormComp textField={"title:"} stringSetter={setTitle} textDisplay='block' textDisplaymd='flex' />}
                    {<FormComp numberField={"price:"} integerSetter={setPrice} numberDisplay='block' numberDisplaymd='flex' />}
                    {<FormComp numberField={"old price:"} integerSetter={setOldPrice} numberDisplay='block' numberDisplaymd='flex' />}
                    {<FormComp numberField={"total Product:"} integerSetter={setTotalQuantity} numberDisplay='block' numberDisplaymd='flex' />}
                    {<FormComp textField={"product code:"} stringSetter={setProductCode} textDisplay='block' textDisplaymd='flex' />}
                    {<FormComp selectField={"category"} selectDisplay='block' selectDisplaymd='flex' stringSetter={setCategory} 
                        selectField1opt1={"Ladies Wear"} selectField1opt2={"Ladies Shoes"} selectField1opt3={"Men Wears"} 
                        selectField1opt4={"Men Shoes"} />}
                    {<FormComp textAreaField={"product details:"} textAreaDisplay='block' textAreaDisplaymd='flex' stringSetter={setDetails} />}
                    ...
                    <div className=' flex justify-center '>
                        {<FormComp button1={<ButtonComp text={"add"} bground={"bg-[#ec8951]"} textFill={"text-white"} performFunction={addProductHandler} />} />}
